...
//Hatch Toggle Variables
  int toggle = 0;
  boolean toggleStick = leftJoystick.getRawButtonPressed(1);
  int i = 0;
  ...
  public void robotPeriodic() {
    ...
    */
    ...
    //Hatch Toggle
    toggleStick = leftJoystick.getRawButtonPressed(1); //Reads the Trigger State
    ...
    if (leftJoystick.getRawButton(2)) { //Elevator Down
      elevatorSpark.set(-.4);
    } else if (leftJoystick.getRawButton(3)) { //Elevator Up
    ...
    //Hatch Intake/Extake
    if(leftJoystick.getRawButton(4)){ //Hatch Intake
      hatchIntake.set(.65);
    } else if (leftJoystick.getRawButton(5)){ //Hatch Extake
      hatchIntake.set(-.65);
    } else {
    ...
    //Compressor
    if (leftJoystick.getRawButton(6)) { //Compressor On
      comp1.setClosedLoopControl(true);
    } else if (leftJoystick.getRawButton(7)) { //Compressor Off
      comp1.setClosedLoopControl(false);
    }

    //Climb Motor
    if (leftJoystick.getRawButton(10)) { //Retract Arms
    climbSpark.set(1);    
    } else if(leftJoystick.getRawButton(11)) { //Extend Arms
    climbSpark.set(-1);
    } else {
    ...
    //Climb Pneumatics
    if (rightJoystick.getRawButton(3)) { //Unclamp Pistons
      climbSol.set(Value.kReverse);
    } else if(rightJoystick.getRawButton(5)) { //Clamp Pistons
